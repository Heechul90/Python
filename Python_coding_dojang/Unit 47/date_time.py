...
## 우리나라에서 실행했다면
## UTC에 9시간을 더한 KST(Korea Standard Time, 한국 표준시)를 사용합니다(UTC+09:00).

# time.localtime(초)

time.localtime(time.time())

# tm_wday는 요일(월요일~일요일, 0~6),
# tm_yday는 1월 1일부터 경과한 일수,
# tm_isdst는 서머타임 여부입니다.
...
# %Y는 연, %m은 월, %d는 일인데,
# '%Y-%m-%d'는 '연-월-일' 포맷이라는 뜻입니다.
# 그리고 %c는 날짜와 시간을 함께 출력합니다.



## 47.4.4 datetime 모듈로 현재 날짜와 시간 구하기
## datetime 모듈의 datetime 클래스를 사용해보겠습니다.
## datetime.datetime으로 현재 날짜와
## 시간을 구할 때는 today 메서드를 사용합니다(현재 시간대 기준, KST).
...
## datetime 모듈로 현재 시간을 구할 때 UTC를 기준으로 구하고 싶다면
## now 메서드에 pytz 모듈로 시간대를 지정해주어야 합니다.

# datetime.datetime.now(시간대객체)

# pip.install pytz     # 설치 후 작동
...
## datetime 객체를 문자열로 만들 수도 있습니다.
## 이때는 strftime 메서드를 사용합니다.
...
## datetime.datetime 객체는
## 연(year), 월(month), 일(day), 시(hour), 분(minute), 초(second), 마이크로초(microsecond) 속성을
## 따로 가져올 수 있습니다.
import time
...
## datetime.datetime 객체에서 datetime.datetime 객체를 빼면
## datetime.timedelta 객체가 나옵니다.
